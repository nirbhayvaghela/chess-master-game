  ...
  socket.on("remove-spectator", async ({ roomId, spectatorId, byUserId }: RemoveSpectatorSchemaType) => {
    try {
      if (!roomId || !spectatorId || !byUserId) {
        return responder.error("error", "roomId, spectatorId, and byUserId are required.");
      }
      ...
      const isAuthorized = room.player1Id === byUserId || room.player2Id === byUserId;
      if (!isAuthorized) {
        return responder.error("error", "Not authorized to remove spectators.");
      }
